...
#[test]
fn ordinary_material_texture_samples_use_lod_bias_or_allowlist() -> io::Result<()> {
    let mut offenders = Vec::new();
    for root in ["shaders/materials", "shaders/modules"] {
        for path in wgsl_files_recursive(root)? {
            ...
            for (line_no, line) in src.lines().enumerate() {
                ...
                if raw_material_texture_sample_allowlisted(&label, line) {
                    continue;
                }
                offenders.push(format!(
                    "{}:{} still uses raw `{}`",
    ...
    assert!(
        offenders.is_empty(),
        "ordinary host material textures must route through texture_sampling helpers or an explicit allowlist:\n  {}",
        offenders.join("\n  ")
    );
    Ok(())
}

fn raw_material_texture_sample_allowlisted(label: &str, line: &str) -> bool {
    if matches!(
        label,
        "shaders/modules/frame/grab_pass.wgsl"
            | "shaders/modules/lighting/light_cookies.wgsl"
            | "shaders/modules/skybox/projection360_material.wgsl"
    ) {
        return true;
    }
    if label == "shaders/modules/xiexe/toon2/lighting.wgsl"
        && line.contains("textureSample(xb::_Ramp")
    {
        return true;
    }
    matches!(
        label,
        "shaders/modules/pbs/displace.wgsl"
            | "shaders/materials/depthprojection.wgsl"
            | "shaders/materials/faceexplodeshader.wgsl"
            | "shaders/materials/fresnel.wgsl"
            | "shaders/materials/fresnellerp.wgsl"
            | "shaders/materials/pbscolorsplat.wgsl"
            | "shaders/materials/pbscolorsplatspecular.wgsl"
            | "shaders/materials/pbscolormaskspecular.wgsl"
            | "shaders/materials/pbsdisplace.wgsl"
            | "shaders/materials/pbsdisplacespecular.wgsl"
            | "shaders/materials/pbsdisplacetransparent.wgsl"
            | "shaders/materials/pbsdisplacespeculartransparent.wgsl"
            | "shaders/materials/pbsintersect.wgsl"
            | "shaders/materials/pbsintersectspecular.wgsl"
            | "shaders/materials/pbslerp.wgsl"
            | "shaders/materials/pbslerpspecular.wgsl"
            | "shaders/materials/pbsmultiuv.wgsl"
            | "shaders/materials/pbsmultiuvspecular.wgsl"
            | "shaders/materials/pbsrim.wgsl"
            | "shaders/materials/pbsrimspecular.wgsl"
            | "shaders/materials/pbsrimtransparent.wgsl"
            | "shaders/materials/pbsrimtransparentspecular.wgsl"
            | "shaders/materials/pbsrimtransparentzwrite.wgsl"
            | "shaders/materials/pbsvoronoicrystal.wgsl"
            | "shaders/materials/pbsvertexcolortransparent.wgsl"
            | "shaders/materials/pbsvertexcolortransparentspecular.wgsl"
    )
}
