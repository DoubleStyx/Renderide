...
//! base, forward additive, and shadow caster passes. The material declares a forward base pass and
//! a depth-only shadow caster pass with culling disabled by default.
//!
...
//#pass type=shadow_caster name=shadow_caster cull=material(off) zwrite=on ztest=main color_mask=0 offset=material(0,0)
@fragment
fn fs_shadow_caster(
    @builtin(front_facing) front_facing: bool,
    @location(1) world_n: vec3<f32>,
    ...
    let _surface = sample_surface(uv0, world_n, world_t, front_facing, color);
}
