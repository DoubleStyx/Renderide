//! FurFX 1.x advanced material bindings and shading.

#define_import_path renderide::fur::classic_advanced
...
struct ClassicAdvancedMaterial {
    _Color: vec4<f32>,
    _SpecColor: vec4<f32>,
    _RimColor: vec4<f32>,
...
@group(1) @binding(0) var<uniform> mat: ClassicAdvancedMaterial;
@group(1) @binding(1) var _MainTex: texture_2d<f32>;
@group(1) @binding(2) var _MainTex_sampler: sampler;
@group(1) @binding(3) var _NoiseTex: texture_2d<f32>;
@group(1) @binding(4) var _NoiseTex_sampler: sampler;
@group(1) @binding(5) var _Cube: texture_cube<f32>;
@group(1) @binding(6) var _Cube_sampler: sampler;
...
fn shell_emission(input: furc::VertexOutput, alpha: f32) -> vec3<f32> {
    ...
    return rim + reflection * max(mat._Reflection, 0.0) * alpha;
}
...
fn fragment_shell(input: furc::VertexOutput) -> vec4<f32> {
    ...
    return shaded_color(input, base_color, alpha, shell_emission(input, alpha), 1.0);
}
