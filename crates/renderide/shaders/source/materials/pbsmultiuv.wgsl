//! Unity PBS multi-UV metallic (`Shader "PBSMultiUV"`).
//!
//! Supports dual albedo/emission maps plus per-map UV set selectors (`0 -> uv0`, `>=1 -> uv1`)
//! using the renderer's available UV channels.
...
@group(1) @binding(13) var _MetallicMap: texture_2d<f32>;
@group(1) @binding(14) var _MetallicMap_sampler: sampler;
...
fn fs_main(
    ...
    let uv_metal = uvu::apply_st(pick_uv(mat._MetallicUV, uv0, uv1, uv2, uv3), mat._MetallicMap_ST);
    ...
    if (uvu::kw_enabled(mat._METALLICMAP)) {
        let mg = textureSample(_MetallicMap, _MetallicMap_sampler, uv_metal);
        metallic = mg.r;
        smoothness = mg.a;
    }
