...
pub(crate) struct WorldMeshForwardEncodeRefs<'a> {
    ...
    /// Resident mesh and texture pools (mutable for mesh stream uploads during draw encoding).
...
impl<'a> WorldMeshForwardEncodeRefs<'a> {
    /// Mutable mesh pool for lazy vertex stream uploads required by some embedded stems.
