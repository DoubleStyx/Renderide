...
fn rgb_to_hsv_no_clip_0( RGB_0 : vec3<f32>) -> vec3<f32>
{
    ...
    var _S5 : vec2<f32> = vec2<f32>(vec2<i32>(i32(0)));
    HSV_0.x = _S5.x;
    HSV_0.y = _S5.y;
...
fn hsv_to_rgb_0( HSV_1 : vec3<f32>) -> vec3<f32>
{
    ...
    var _S6 : f32 = HSV_1.z;
    var _S7 : f32 = HSV_1.y;
    var var_1_0 : f32 = _S6 * (1.0f - _S7);
    var _S8 : f32 = var_h_0 - var_i_0;
    var var_2_0 : f32 = _S6 * (1.0f - _S7 * _S8);
    var var_3_0 : f32 = _S6 * (1.0f - _S7 * (1.0f - _S8));
    var RGB_1 : vec3<f32>;
    if(var_i_0 == 0.0f)
    {
        RGB_1 = vec3<f32>(_S6, var_3_0, var_1_0);
    }
    else
    {
        if(var_i_0 == 1.0f)
        {
            RGB_1 = vec3<f32>(var_2_0, _S6, var_1_0);
        }
        else
        {
            if(var_i_0 == 2.0f)
            {
                RGB_1 = vec3<f32>(var_1_0, _S6, var_3_0);
            }
            else
            {
                if(var_i_0 == 3.0f)
                {
                    RGB_1 = vec3<f32>(var_1_0, var_2_0, _S6);
                }
                else
                {
                    if(var_i_0 == 4.0f)
                    {
                        RGB_1 = vec3<f32>(var_3_0, var_1_0, _S6);
                    }
                    else
                    {
                        RGB_1 = vec3<f32>(_S6, var_1_0, var_2_0);
                    }
                }
            }
        }
...
@fragment
fn frag( _S9 : pixelInput_0, @builtin(position) pos_0 : vec4<f32>) -> pixelOutput_0
{
    if(USC_RECTCLIP_0)
    {
        if((UnityGet2DClipping_0(_S9.position_1, globalParams_0._Rect_0) - 0.10000000149011612f) < 0.0f)
        {
            discard;
        }
    }
    var _S10 : vec4<f16> = vec4<f16>((textureSample((_GrabTexture_0), (unity_usc_resonite_macro_sampler_0), (_S9.grabPos_0.xy / vec2<f32>(_S9.grabPos_0.w)))));
    var c_0 : vec4<f16> = _S10;
    var _S11 : vec3<f32> = rgb_to_hsv_no_clip_0(vec3<f32>(_S10.xyz)) * globalParams_0._HSVMul_0.xyz + globalParams_0._HSVOffset_0.xyz;
    var hsv_0 : vec3<f32> = _S11;
    hsv_0[i32(0)] = fract(_S11.x);
    hsv_0[i32(1)] = saturate(hsv_0.y);
    var _S12 : vec3<f16> = vec3<f16>(hsv_to_rgb_0(hsv_0));
    c_0.x = _S12.x;
    c_0.y = _S12.y;
    c_0.z = _S12.z;
    var _S13 : pixelOutput_0 = pixelOutput_0( c_0 );
    return _S13;
}
