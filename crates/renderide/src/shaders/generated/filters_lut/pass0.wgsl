...
@binding(29) @group(0) var U_SecondaryLUT_0 : texture_3d<f32>;

@binding(30) @group(0) var sampler_SecondaryLUT_0 : sampler;

@binding(31) @group(0) var _GrabTexture_0 : texture_2d<f32>;

@binding(32) @group(0) var sampler_GrabTexture_0 : sampler;
...
fn frag( _S2 : pixelInput_0, @builtin(position) pos_0 : vec4<f32>) -> pixelOutput_0
{
    ...
    var _S3 : vec4<f16> = vec4<f16>((textureSample((_GrabTexture_0), (unity_usc_resonite_macro_sampler_0), (_S2.grabPos_0.xy / vec2<f32>(_S2.grabPos_0.w)))));
    var c_0 : vec4<f16> = _S3;
    var _S4 : f16 = max(max(_S3.x, _S3.y), max(_S3.z, 1.0h));
    if(_S4 > 1.0h)
    {
        var _S5 : vec3<f16> = c_0.xyz / vec3<f16>(_S4);
        c_0.x = _S5.x;
        c_0.y = _S5.y;
        c_0.z = _S5.z;
    }
    var _S6 : vec3<f16> = vec3<f16>(mix((textureSampleLevel((_LUT_0), (unity_usc_resonite_macro_sampler_0), (vec3<f32>(vec4<f16>(c_0.xyz, 0.0h).xyz)), (f32(vec4<f16>(c_0.xyz, 0.0h).w)))).xyz, (textureSampleLevel((U_SecondaryLUT_0), (unity_usc_resonite_macro_sampler_0), (vec3<f32>(vec4<f16>(c_0.xyz, 0.0h).xyz)), (f32(vec4<f16>(c_0.xyz, 0.0h).w)))).xyz, vec3<f32>(globalParams_0._Lerp_0)));
    c_0.x = _S6.x;
    c_0.y = _S6.y;
    c_0.z = _S6.z;
    var _S7 : vec3<f16> = c_0.xyz * vec3<f16>(_S4);
    c_0.x = _S7.x;
    c_0.y = _S7.y;
    c_0.z = _S7.z;
    var _S8 : pixelOutput_0 = pixelOutput_0( c_0 );
    return _S8;
}
