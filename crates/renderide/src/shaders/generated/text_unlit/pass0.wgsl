...
{
    var atlasColor_0 : vec4<f32> = (textureSample((_FontAtlas_0), (unity_usc_resonite_macro_sampler_0), (_S1.uv_0)));
    ...
    if(_S3)
    {
        ...
        var sigDist_1 : f32 = sigDist_0 + (globalParams_0._FaceDilate_0 + _S1.extraData_0.x);
        var _S4 : f32 = max(dot(globalParams_0._Range_0, vec2<f32>(0.5f) / (fwidth((_S1.uv_0)))), 1.0f);
        var glyphLerp_0 : f32 = saturate(mix(sigDist_1 * _S4, sigDist_1, globalParams_0._FaceSoftness_0) + 0.5f);
        if((max(glyphLerp_0, globalParams_0._BackgroundColor_0.w) - 0.00100000004749745f) < 0.0f)
        {
            discard;
        }
        var fillColor_0 : vec4<f32> = globalParams_0._TintColor_0 * _S1.color_0;
        if(USC_OUTLINE_0)
        {
            var outlineDist_0 : f32 = sigDist_1 - (globalParams_0._OutlineSize_0 + _S1.extraData_0.y);
            outColor_0 = mix(globalParams_0._OutlineColor_0 * vec4<f32>(1.0f, 1.0f, 1.0f, _S1.color_0.w), fillColor_0, vec4<f32>(saturate(mix(outlineDist_0 * _S4, outlineDist_0, globalParams_0._FaceSoftness_0) + 0.5f)));
        }
        else
        {
            outColor_0 = fillColor_0;
        }
        outColor_0 = mix(globalParams_0._BackgroundColor_0 * _S1.color_0, outColor_0, vec4<f32>(glyphLerp_0));
    }
    else
    {
        if(USC_RASTER_0)
        {
            var outColor_1 : vec4<f32> = atlasColor_0 * _S1.color_0;
            if((outColor_1.w - 0.00100000004749745f) < 0.0f)
            {
                discard;
            }
            outColor_0 = outColor_1;
        }
        else
        {
            outColor_0 = _S2;
    ...
    var _S5 : pixelOutput_0 = pixelOutput_0( outColor_0 );
    return _S5;
}
