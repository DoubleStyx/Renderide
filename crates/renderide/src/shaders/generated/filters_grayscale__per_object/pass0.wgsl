...
@binding(27) @group(0) var _Gradient_0 : texture_2d<f32>;

@binding(28) @group(0) var sampler_Gradient_0 : sampler;
...
fn frag( _S2 : pixelInput_0, @builtin(position) pos_0 : vec4<f32>) -> pixelOutput_0
{
    ...
    var grayscale_0 : f16 = f16(f32(_S3.x) * globalParams_0._RatioR_0 + f32(_S3.y) * globalParams_0._RatioG_0 + f32(_S3.z) * globalParams_0._RatioB_0);
    var newColor_0 : vec3<f16>;
    if(USC_GRADIENT_0)
    {
        newColor_0 = vec3<f16>((textureSampleLevel((_Gradient_0), (unity_usc_resonite_macro_sampler_0), (vec2<f32>(f32(grayscale_0), 0.0f)), (0.0f))).xyz);
    }
    else
    {
        newColor_0 = vec3<f16>(grayscale_0);
    }
    var _S4 : vec3<f16> = vec3<f16>(mix(vec3<f32>(c_0.xyz), vec3<f32>(newColor_0), vec3<f32>(globalParams_0._Lerp_0)));
    c_0.x = _S4.x;
    c_0.y = _S4.y;
    c_0.z = _S4.z;
