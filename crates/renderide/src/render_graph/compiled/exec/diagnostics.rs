...
impl CommandEncodingDiagnostics {
    ...
    pub(super) fn plot(&self) {
        let sample = crate::profiling::CommandEncodingProfileSample {
            ...
            upload_traffic: self.upload_stats.traffic,
            upload_arena: self.upload_stats.arena,
        ...
        crate::profiling::plot_frame_upload_arena(
            &crate::profiling::FrameUploadArenaProfileSample {
                traffic: self.upload_stats.traffic,
                arena: self.upload_stats.arena,
            },
        );
    }
    ...
    pub(super) fn record_flight_event(&self, gpu: &GpuContext) {
        gpu.record_gpu_flight_event(GpuFlightEventKind::RenderGraphSubmit {
            ...
            upload_bytes: self.upload_stats.traffic.bytes,
            transient_texture_misses: self.transient_delta.texture_misses,
            transient_buffer_misses: self.transient_delta.buffer_misses,
        });
    }
    ...
    pub(super) fn log_if_slow(&self) {
        ...
        logger::warn!(
            ...
            self.upload_stats.traffic.writes,
            self.upload_stats.traffic.bytes,
            self.upload_stats.traffic.staged_writes,
            self.upload_stats.traffic.fallback_writes,
            self.upload_stats.arena.acquire.persistent_staging_bytes,
            self.upload_stats.arena.acquire.temporary_staging_bytes,
            self.upload_stats.arena.acquire.persistent_slot_reuses,
            self.upload_stats.arena.acquire.persistent_slot_grows,
            self.upload_stats.arena.acquire.temporary_staging_fallbacks,
            self.upload_stats
                .arena
                .acquire
                .oversized_queue_fallback_writes,
            self.upload_stats.arena.pressure.capacity_bytes,
            self.upload_stats.arena.pressure.free_slots,
            self.upload_stats.arena.pressure.in_flight_slots,
            self.upload_stats.arena.pressure.remapping_slots,
            self.pre_resolve_ms,
