...
use execute_helpers::{
    ...
    resolve_forward_msaa_views, ForwardPassEncodeFrame, ForwardPassEncodeViews,
};
...
impl RenderPass for WorldMeshForwardPass {
    ...
    fn execute(&mut self, ctx: &mut RenderPassContext<'_, '_, '_>) -> Result<(), RenderPassError> {
        ...
        let color_view = msaa_views
            .as_ref()
            .map(|v| &v.color_view)
            .unwrap_or(bb);
        let depth_raster = msaa_views
            .as_ref()
            .map(|v| &v.depth_view)
            .unwrap_or(depth);
