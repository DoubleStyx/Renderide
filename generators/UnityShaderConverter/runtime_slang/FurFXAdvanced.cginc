// Resonite FurFX vendored shader (see third_party/Resonite.UnityShaders/.../FurFXAdvanced.cginc).
//
// Renderide: This file shadows the Resonite copy via slangc -I order (runtime_slang first). Do not edit third_party.
// Keep in sync when updating Resonite.UnityShaders.
//
// Slang is stricter than Unity HLSL about implicit conversions:
// - Assign float4 (matrix × vector) to half2: take .xy and cast to half2.
// - Assign float3 expression to scalar alpha: use a single channel (.x) for grayscale noise.
// - compound *= / += on half3 with half4 RHS: use .rgb on the varying.
...
v2f VertexProgram (appdata v)
{
	...
	o.uv2.xy = half2(mul(UNITY_MATRIX_TEXTURE1, float4(v.texcoord + znormal * 0.0011 * FUR_MULTIPLIER)).xy);
...
fixed4 frag (v2f i) : COLOR
{
	...
	o.a = clamp(noise.x - (FUR_MULTIPLIER*FUR_MULTIPLIER)*_EdgeFade,0,1);
